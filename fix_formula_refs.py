...
for row in range(1, ws_edu.max_row + 1):
    for col in range(1, ws_edu.max_column + 1):
        ...
        if source_cell.data_type == 'f':
            # 调整公式中的行号引用
            offset = edu_start_row - 1
            adjusted_formula = adjust_formula_row(source_cell.value, offset)
...
print(f"\n   教学楼汇总行 (第268行):")
for col in [20, 21, 24]:  # T, U, X列
    cell = ws_summary.cell(row=268, column=col)
    print(f"     {get_column_letter(col)}268: {cell.value}")

# 教学楼第一行数据
print(f"\n   教学楼第一行数据 (第269行):")
for col in [9, 10, 11]:  # I, J, K列
    cell = ws_summary.cell(row=269, column=col)
    print(f"     {get_column_letter(col)}269: {cell.value}")
