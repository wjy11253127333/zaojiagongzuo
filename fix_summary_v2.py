...
for col in range(1, ws_edu.max_column + 1):
    source_cell = ws_edu.cell(row=edu_summary_row, column=col)
    target_cell = ws_summary.cell(row=edu_start_row, column=col)
    ...
    if source_cell.data_type == 'f':
        print(f"   {get_column_letter(col)}{edu_start_row}: {source_cell.value}")
...
for row in range(edu_data_start, ws_edu.max_row + 1):
    source_row = row
    target_row = edu_start_row + (row - edu_data_start) + (edu_summary_row - edu_data_start + 1)  # 调整目标行号
...
for merged_range in ws_edu.merged_cells.ranges:
    ...
    if min_row >= edu_data_start:  # 只复制数据部分的合并单元格
        new_min_row = edu_start_row + (min_row - edu_data_start) + (edu_summary_row - edu_data_start + 1)
        new_max_row = edu_start_row + (max_row - edu_data_start) + (edu_summary_row - edu_data_start + 1)
...
# 综合楼汇总行
print(f"\n   综合楼汇总行 (第4行):")
cell = ws_summary.cell(row=4, column=20)
print(f"     T4: {cell.value}")

# 教学楼汇总行
print(f"\n   教学楼汇总行 (第268行):")
cell = ws_summary.cell(row=268, column=20)
