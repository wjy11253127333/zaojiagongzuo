...
temp_path = r"c:\Users\aa\Desktop\造价清单文件标准化\temp_copy.xlsx"
...
def create_cost_summary_sheet(wb):
    ...
    for idx, (category, data) in enumerate(summary_by_category.iterrows(), 1):
        ...
        ws.cell(row=row_num, column=6, value=data['审核合价'] if data['审核合价'] > 0 else 0)
        ws.cell(row=row_num, column=7, value=data['偏差（结算-合同）'])
        ws.cell(row=row_num, column=8, value=data['偏差率（%）'] / 100)
        ws.cell(row=row_num, column=9, value=data['成本降低额'] if data['审核合价'] > 0 else 0)
        ws.cell(row=row_num, column=10, value=data['成本降低率（%）'] / 100 if data['审核合价'] > 0 else 0)
    ...
    ws.cell(row=row_num, column=6, value=total_audit if total_audit > 0 else 0)
    ...
    ws.cell(row=row_num, column=8, value=total_deviation_rate / 100)

    total_reduction = total_contract - total_audit
    ws.cell(row=row_num, column=9, value=total_reduction if total_audit > 0 else 0)
    total_reduction_rate = (total_reduction / total_contract * 100) if total_contract > 0 and total_audit > 0 else 0
    ws.cell(row=row_num, column=10, value=total_reduction_rate / 100 if total_audit > 0 else 0)
...
def create_comprehensive_analysis_sheet(wb):
    ...
    ws.cell(row=6, column=2, value=total_audit if total_audit > 0 else 0)
    ...
    ws.cell(row=8, column=2, value=deviation_rate / 100)
    ...
    for category, data in summary_by_category.iterrows():
        ...
        ws.cell(row=row, column=2, value=proportion / 100)
...
def create_detailed_data_sheet(wb, df):
    ...
    for idx, row_data in df.iterrows():
        ...
        ws.cell(row=row_num, column=11, value=row_data['审核合价'] if row_data['审核合价'] > 0 else 0)
...
try:
    ...
    try:
        ...
        print(f"   ⚠️ 无法复制到目标位置: {e}")
        print(f"   文件仍保留在: {temp_path}")
